...
export const Card: React.FC<{ className?: string; children: React.ReactNode }> = ({ className, children }) => (
  <div className={cx("bg-white border border-gray-200 rounded-2xl p-4 shadow-card", className)}>{children}</div>
);
  ...
  const variantStyles =
    variant === "primary"
      ...
      : variant === "secondary"
        ...
        : "border border-gray-200 text-gray-900 hover:bg-gray-50 focus:ring-primary-500";
...
export const Input: React.FC<InputHTMLAttributes<HTMLInputElement>> = ({ className, ...props }) => (
  <input
    className={cx(
      "w-full rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm text-gray-900 shadow-sm focus:border-primary-500 focus:ring-2 focus:ring-primary-200",
      className
    )}
    {...props}
  />
...
export const Table: React.FC<{ headers: string[]; children: React.ReactNode; className?: string }> = ({ headers, children, className }) => (
  <div className={cx("overflow-hidden rounded-xl border border-gray-200 bg-white", className)} role="table">
    <div className="grid grid-cols-[repeat(auto-fit,minmax(120px,1fr))] gap-2 bg-gray-50 px-4 py-2 text-sm font-semibold text-gray-700" role="rowgroup">
      {headers.map((h) => (
...
export const TableRow: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <div className="grid grid-cols-[repeat(auto-fit,minmax(120px,1fr))] gap-2 px-4 py-3 text-sm text-gray-900" role="row">
    {children}
  </div>
);
